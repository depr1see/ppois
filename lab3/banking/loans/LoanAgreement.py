...
from banking.exceptions import LimitExceededException, LoanDefaultException, OverdraftException

@dataclass
class LoanAgreement:
    agreement_id: str = ""
    principal: float = 0.0
    collateral: CollateralItem | None = None

    def update_loan_agreement(self, note: str = "", delta: float | None = None) -> str:
        if note and (self.principal <= 0 or self.collateral is None):
            raise LoanDefaultException("Loan agreement missing principal or collateral.")
    ...
    def describe_loan_agreement(self) -> str:
        parts = []
        ...
        return f"LoanAgreement[" + ", ".join(parts) + "]"
