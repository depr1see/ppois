...
from banking.exceptions import InvalidScheduleException, LimitExceededException, OverdraftException

@dataclass
class RepaymentSchedule:
    schedule_id: str = ""
    due_dates: list[str] = field(default_factory=list)
    agreement: LoanAgreement | None = None

    def update_repayment_schedule(self, note: str = "", delta: float | None = None) -> str:
        if note and not self.due_dates:
            raise InvalidScheduleException("Repayment schedule must contain due dates.")
    ...
    def describe_repayment_schedule(self) -> str:
        parts = []
        ...
        return f"RepaymentSchedule[" + ", ".join(parts) + "]"
