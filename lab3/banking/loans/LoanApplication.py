...
from banking.exceptions import LimitExceededException, OverdraftException, PaymentRejectedException
...
class LoanApplication:
    ...
    def update_loan_application(self, note: str = "", delta: float | None = None) -> str:
        if note and (self.amount <= 0 or self.customer is None):
            raise PaymentRejectedException("Loan application must include amount and customer.")
    ...
    def describe_loan_application(self) -> str:
        parts = []
        ...
        return f"LoanApplication[" + ", ".join(parts) + "]"
