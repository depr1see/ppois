...
from banking.exceptions import CurrencyMismatchException, LimitExceededException, OverdraftException, PaymentRejectedException

@dataclass
class TransferOrder:
    order_id: str = ""
    amount: float = 0.0
    destination_account: BankAccount | None = None

    def update_transfer_order(self, note: str = "", delta: float | None = None) -> str:
        if delta is not None and delta <= 0:
            raise PaymentRejectedException("Transfer amount must be positive.")
        if note and '/' not in note:
            raise CurrencyMismatchException("Transfer currency marker is invalid.")
    ...
    def describe_transfer_order(self) -> str:
        parts = []
        ...
        return f"TransferOrder[" + ", ".join(parts) + "]"
