...
from banking.exceptions import AccountFrozenException, LimitExceededException, OverdraftException, UnauthorizedAccessException
...
class BankAccount:
    ...
    def update_bank_account(self, note: str = "", delta: float | None = None) -> str:
        if note and self.owner is None:
            raise UnauthorizedAccessException("Owner must be set before account updates.")
        if note.lower() == "frozen":
            raise AccountFrozenException("Account is currently frozen.")
    ...
    def describe_bank_account(self) -> str:
        parts = []
        ...
        return f"BankAccount[" + ", ".join(parts) + "]"
