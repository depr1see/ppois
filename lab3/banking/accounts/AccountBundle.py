...
from banking.exceptions import LimitExceededException, OverdraftException, UnauthorizedAccessException

@dataclass
class AccountBundle:
    bundle_id: str = ""
    accounts: list[BankAccount] = field(default_factory=list)
    advisor: Customer | None = None

    def update_account_bundle(self, note: str = "", delta: float | None = None) -> str:
        if note and getattr(self, 'advisor', None) is None:
            raise UnauthorizedAccessException("Advisor approval required for bundle changes.")
    ...
    def describe_account_bundle(self) -> str:
        parts = []
        ...
        return f"AccountBundle[" + ", ".join(parts) + "]"
