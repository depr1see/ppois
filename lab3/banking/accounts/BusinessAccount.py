...
from banking.exceptions import AccountFrozenException, LimitExceededException, OverdraftException

@dataclass
class BusinessAccount:
    account_number: str = ""
    tax_id: str = ""
    controllers: list[EmployerInfo] = field(default_factory=list)

    def update_business_account(self, note: str = "", delta: float | None = None) -> str:
        if note.lower().startswith("freeze") and not self.controllers:
            raise AccountFrozenException("Controllers must approve freeze.")
    ...
    def describe_business_account(self) -> str:
        parts = []
        ...
        return f"BusinessAccount[" + ", ".join(parts) + "]"
