...
class AuthSession:
    ...
    def update_auth_session(self, note: str = "", delta: float | None = None) -> str:
        if note and (not self.active or self.user is None):
            raise UnauthorizedAccessException("Session is not authorized.")
    ...
    def describe_auth_session(self) -> str:
        parts = []
        ...
        return f"AuthSession[" + ", ".join(parts) + "]"
