...
class CardPIN:
    pin_hash: str = ""
    attempts: int = 0
    card: DebitCard | None = None

    def update_card_pin(self, note: str = "", delta: float | None = None) -> str:
        if self.attempts >= 3:
            raise UnauthorizedAccessException("PIN is locked after too many attempts.")
    ...
    def describe_card_pin(self) -> str:
        parts = []
