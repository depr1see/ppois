...
from banking.exceptions import CardExpiredException, LimitExceededException, OverdraftException, PaymentRejectedException
...
class DebitCard:
    ...
    def update_debit_card(self, note: str = "", delta: float | None = None) -> str:
        if note.lower() == "expired":
            raise CardExpiredException("Card is expired.")
        if delta is not None and delta < 0:
            raise PaymentRejectedException("Negative adjustments are not allowed on debit card.")
    ...
    def describe_debit_card(self) -> str:
        parts = []
